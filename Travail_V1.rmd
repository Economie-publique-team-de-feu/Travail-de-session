...
$$ U(Y_i,L_i) = \alpha_{1i}\ln(\bar{L}-L) +\alpha_2\ln(\bar{L}-L)^2+ \alpha_3\ln(Y_i)+\alpha_4\ln(Y_i)^2 + \gamma_0(L=0) + \gamma_f(L\in[1750,2000])$$

où $Y_i$ est le revenu disponible (la consommation), $L_i$ est la quantité de travail (annuelle, en heures) et $\bar{L}$ est le nombre maximal d'heures qu'un individu peut travailler annuellement. Ainsi spécifié, le terme $\bar{L}-L$ est le nombre d'heures annuelles de loisir consommé. 
...
Pour coder la fonction d'utilité, la vecteur de coefficients est noté $\theta = (\alpha_2, \alpha_3, \alpha_4, \gamma_0, \gamma_f)$ et le coefficient $\alpha_{1i}$ est codé tel quel, car il a sa propre fonction basée sur les caractéristiques des individus. 

```{r}
Utilite <- function(i,L,alpha,beta,theta){
# Fonction d'utilité du travail et de la consommation
      util <- alpha1(i,beta)*log(Lmax-L)+ theta[1]*log((Lmax-L)^2)+  theta[2]*log(Y(i,L)) + theta[3]*log((Y(i,L))^2)+ theta[4]*as.numeric(L==0) + theta[5]*as.numeric(which(L>1750 & L<2000))
  return(util)
}
...
L'utilité est donnée par une fonction qui prend comme argument l'individu *i*, le nombre d'heures travaillées *L* et le paramètre de caractéristiques individuelles *alpha1*. *Lmax* est le nombre d'heures travaillées maximales.

## Contrainte budgétaire

La consommation (ou revenu disponible) est une fonction de l'individu *i* et du nombre travaillé et est donnée par la contrainte budgétaire:

$$
w_iL-t(w_iL_i)\geq Y_i
$$

qui sera saturée à l'optimum. La fonction de taxation $t(w_iL_i)$ est  une fonction non-linéaire du revenu de travail brut.  $w_i$ est le salaire horaire pour chaque individu *i*. Le code R associé à la contrainte budgétaire est donné par:
...
$$ U(Y_i,L_i) = \alpha_{1i}\ln(\bar{L}-L) +\alpha_2\ln(\bar{L}-L)^2+ \alpha_3\ln(w_iL_i-t(w_iL_i))+\alpha_4\ln(w_iL_i-t(w_iL_i))^2 + \gamma_0(L=0) + \gamma_f(L\in[1750,2000])$$

En supposant que *L* est une variable continue et que $t(\cdot)$ est différentiable (et en supposant que $\gamma_0=\gamma_f=0$), on peut prendre les conditions de premier ordre et réécrire:

$$
\frac{\bar{L}-L}{Y_i }=\frac{\alpha_i}{w_i(1-t'(w_iL))}
$$
...
* *Lmax* est le nombre maximal d'heures travaillées par année, soit 85 heures par semaine.
* *weeks* est le nombre de semaines de travail par année.
* *lincr* est le nombre minimal d'heures annuelles de travail que l'individu peut ajuster, soit des incréments de 5 heures par semaine.
```{r}
Lmax <- 4250
weeks <- 50
lincr <- 250
