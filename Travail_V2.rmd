...
transferts <- function(inc, i){
  ...
  #Crédit TPS-TVH
  TPS_TVH[i] <- 296 # montant de base
  if (c[i] == 1 | ce[i] == 1 | fm[i] == 1) { 
    TPS_TVH[i] <- TPS_TVH[i] + 296 # montant si personne a charge ou monoparental
    if (ce[i] == 1) {
      TPS_TVH[i] <- TPS_TVH[i] + 155 # montant si couple a un enfant
    }
  }
  if (ps[i] == 1) {
    TPS_TVH[i] <- TPS_THV[i] + min(155, max(0, 0.02*(inc-9590))) # prime au travail pour celibataire
  } else if (fm[i] == 1) {
    TPS_TVH[i] <- TPS_TVH[i] + 155 # prime au travail si monoparental
  }
  if (inc > 38507) {
    TPS_TVH[i] <- max(0, TPS_TVH[i] - 0.05*(inc-38507)) # déduction pour haut revenu 
  } 
  if (c[i] == 1 | ce[i] == 1 ) { 
    TPS_TVH[i] <- TPS_TVH[i]/2 # montant final divisé par 2 si en couple 
  }
  ...
  if (ps[i] == 1){
    if (inc > 2400 & inc < 19456){  # celibataire
      PrimeTravail[i] <- 0.105*(inc-2400) - max(0, 0.1*(inc-10720))
    } else {
      PrimeTravail[i] <- 0
    }
  } else if (fm[i] == 1){
    if (inc > 2400 & inc < 36680){ # monoparental
      PrimeTravail[i] <- 0.30*(inc-2400) - max(0, 0.1*(inc-10720))
    } else {
      PrimeTravail[i] <- 0
    }
  } else if (c[i] == 1){
    if (inc > 3600/2 & inc < 30217/2){ # couple, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      PrimeTravail[i] <- (0.105*(inc-3600/2) - max(0, 0.1*(inc-16584/2))) / 2 # divisé en 2 pour couple
    } else {
      PrimeTravail[i] <- 0
    }
  } else if (ce[i] == 1){
    if (inc > 3600/2 & inc < 49044/2){ # couple avec enfant, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      PrimeTravail[i] <- (0.25*(inc-3600/2) - max(0, 0.1*(inc-16584/2))) / 2 # divisé en 2 pour couple
    } else {
      PrimeTravail[i] <- 0
    }
  ...
  if (ps[i] == 1){
    if (inc > 2400 & inc < 23458){ # celibataire
      AllocationTravailleur[i] <- 0.274*(inc-2400) - max(0, 0.2*(inc-12060))
    } else {
      AllocationTravailleur[i] <- 0
    }
  } else if (fm[i] == 1){
    if (inc > 2400 & inc < 18305){ # monoparental
      AllocationTravailleur[i] <- 0.15*(inc-2400) - max(0, 0.2*(inc-12065))
    } else {
      AllocationTravailleur[i] <- 0
    }
  } else if (c[i] == 1){
    if (inc > 3600/2 & inc < 36308/2){ # couple, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      AllocationTravailleur[i] <- (0.274*(inc-3600/2) - max(0, 0.2*(inc-18520/2))) / 2
    } else {
      AllocationTravailleur[i] <- 0
    }
  } else if (ce[i] == 1){
    if (inc > 3600/2 & inc < 27629/2){ # couple avec enfant, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      AllocationTravailleur[i] <- (0.14*(inc-3600/2) - max(0, 0.2*(inc-18540/2))) / 2
    } else {
      AllocationTravailleur[i] <- 0
    }
  }
  ...
  if (ce[i] == 1 | fm[i] ==1){   # Aucune distinction entre famille monoparentale et couple
    if (prescolaire[i] == 1) {
      ACE[i] <- 6765 # montant plus élevé si enfant à moins de 6
    } else {
      ACE[i] <- 5708 # on suppose qu'il n'y a qu'un seul enfant
    }
    if (inc > 31711) { # premier seul de réduction (marginal)
      ACE[i] <- ACE[i] - 0.07*min(36997, inc - 31711) # 7% pour revenu entre 31711 et 68708
    }
    if (inc > 68708) { # deuxième seuil
      ACE[i] <- ACE[i] - 0.032*inc - 68708 # 3,2% pour revenu entre 31711 et 68708
    }
    ACE[i] <- max(ACE[i], 0) # on s'assure que la valeur n'est pas négative
    if (ce[i] == 1) {
      ACE[i] <- ACE[i]/2 # on divise le montant pas deux pour un couple
    }
  } else {
      ACE[i] <- 0 #aucune allocation si pas d'enfant
  }
  
  
  #Allocation famille
  if (fm[i] == 1) {
    AllocationFamille[i] <- 2515 + 882 # prime de base plus prime pour monoparental
    if (inc > 36256) {
      AllocationFamille[i] <- AllocationFamille[i] - 0.04*(inc-36256) # déduction pour haut revenu
    }
    AllocationFamille[i] <- max(AllocationFamille[i], 1352) # prime minimum
  } else if (ce[i] == 1) {
    AllocationFamille[i] <- 2515 # montant de base pour un enfant
    if (inc > 49842/2) { # les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      AllocationFamille[i] <- AllocationFamille[i] - 0.04*(inc-49842/2) # déduction pour haut revenu
    }
    AllocationFamille[i] <- max(AllocationFamille[i], 1000) / 2 # prime minimum, divisée par 2
  } else {
    AllocationFamille[i] <- 0
  }
  
  #Crédit solidarité
  if (ps[i] == 1) {
    CreditSolidarite[i] <- 297+141+(577) # montant de base
    if (inc > 35400) { # déduction pour revenu élevé
      # montant versé ne peut pas être inferieur à 0 ou au montant de la composante TVQ
      CreditSolidarite[i] <- max(0, 297 + 141 - 0.03*(inc-35400),  CreditSolidarite[i] - 0.06*(inc-35400))
    } 
  } else if (fm[i] == 1) { 
    CreditSolidarite[i] <- 297+141+(577+123) # montant de base
    if (inc > 35400) { # déduction pour revenu élevé
      # montant versé ne peut pas être inferieur à 0 ou au montant de la composante TVQ
      CreditSolidarite[i] <- max(0, 297 + 141 - 0.03*(inc-35400),  CreditSolidarite[i] - 0.06*(inc-35400))
    } 
  } else if (c[i] == 1) {
    CreditSolidarite[i] <- 297+297+(699) # montant de base
    if (inc > 35400/2) { # déduction pour revenu élevé, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      # montant versé ne peut pas être inferieur à 0 ou au montant de la composante TVQ
      CreditSolidarite[i] <- max(0, 297 + 297 - 0.03*(inc-35400/2),  CreditSolidarite[i] - 0.06*(inc-35400/2)) / 2
      # divisé par 2 pour un couple
    }
  } else if (ce[i] == 1) {
    CreditSolidarite[i] <- 297+297+(699+123) # montant de base
    if (inc > 35400/2) { # déduction pour revenu élevé, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      # montant versé ne peut pas être inferieur à 0 ou au montant de la composante TVQ
      CreditSolidarite[i] <- max(0, 297 + 297 - 0.03*(inc-35400/2),  CreditSolidarite[i] - 0.06*(inc-35400/2)) / 2
...
thetahat <- rep(0.01, 12)

out <- optim(thetahat, fn=distrhours) # minimise la fonction gmmfct prenant theta comme valeur de départ.
