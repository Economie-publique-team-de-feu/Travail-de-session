...
transferts <- function(inc, i){
  ...
  if (ps[i] == 1){
    ...
    if (inc > 3600/2 & inc < 30217/2){ # couple, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      PrimeTravail <- (0.105*(inc-3600/2) - max(0, 0.1*(inc-16584/2))) / 2 # divisé en 2 pour couple
    } else {
      PrimeTravail <- 0
    }
  } else if (ce[i] == 1){
    if (inc > 3600/2 & inc < 49044/2){ # couple avec enfant, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      PrimeTravail <- (0.25*(inc-3600/2) - max(0, 0.1*(inc-16584/2))) / 2 # divisé en 2 pour couple
    } else {
      PrimeTravail <- 0
    }
  ...
  if (ps[i] == 1){
    ...
    if (inc > 3600/2 & inc < 36308/2){ # couple, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      AllocationTravailleur <- (0.274*(inc-3600/2) - max(0, 0.2*(inc-18520/2))) / 2
    } else {
      AllocationTravailleur <- 0
    }
  } else if (ce[i] == 1){
    if (inc > 3600/2 & inc < 27629/2){ # couple avec enfant, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      AllocationTravailleur <- (0.14*(inc-3600/2) - max(0, 0.2*(inc-18540/2))) / 2
    } else {
      AllocationTravailleur <- 0
    }
  }
  ...
  if (fm[i] == 1) {
    ...
    if (inc > 49842/2) { # les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      AllocationFamille <- AllocationFamille - 0.04*(inc-49842/2) # déduction pour haut revenu
    }
  ...
  if (ps[i] == 1) {
    ...
    if (inc > 35400/2) { # déduction pour revenu élevé, les seuil sont divisé par deux puisque le montant représente les revenus totaux du couple
      # montant versé ne peut pas être inferieur à 0 ou au montant de la composante TVQ
      CreditSolidarite <- max(0, 297 + 297 - 0.03*(inc-35400/2),  CreditSolidarite - 0.06*(inc-35400/2)) / 2
