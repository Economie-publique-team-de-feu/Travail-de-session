...
Pour coder la fonction d'utilité, le vecteur de coefficients est noté $\theta = (\alpha_2, \alpha_3, \alpha_4, \gamma_0, \gamma_f, \sigma_L)$ où $\sigma^2_L$ est l'écart-type des heures travaillées. Le coefficient $\alpha_{1i}$ est codé dans une fonction séparée *alpha1*, qui elle dépend d'un vecteur de coefficients $\beta=(\beta_0, \beta_1, \beta_2, \beta_3, \beta_4, \beta_5)$.
...
# calcule l'utilité marginale de l'individu i par incréments de incr heures de travail
  lsup <- seq(0,Lmax,by=incr) # vecteur des heures travaillées
  util <- sapply(lsup,function(x) Utilite(i,x,beta,theta)) # évalue l'utilité pour chaque niveau d'heures travaillées
...
Loptimal <- function(lsup, util){
  position <- which(util==max(util)) #utilité maximale
...
transferts <- function(inc,i){
  ...
  #total CT
  TPS_TVH<-ctps*ps[i]+(ctc*c[i])/2+ctfm*fm[i]+(ctce*ce[i])/2
  ...
  if (3600/2<inc & inc<30217/2) ptc <- 1426.90/2 else ptc <- 0
  if (2400<inc & inc<35680) ptfm <- 2539.2 else ptfm <- 0
  if (3600/2<inc & inc<49044/2) ptce <- 3303/2 else ptce <- 0
  #total PT
  ...
  if (inc<36308/2) (if (inc>18520/2) actc <- min(3558/2, ((inc-3600/2)*0.274)-((inc-18520/2)*0.20)) else actc <-3558/2) else actc <- 0
  if (inc<18305) (if (inc>12065) actfm <- min(1248, ((inc-2400)*0.15)-((inc-12065)*0.20)) else actfm <-1248) else actfm <- 0
  if (inc<27629/2) (if (inc>18540/2) actce <- min(1818/2, ((inc-3600/2)*0.14)-((inc-18540/2)*0.20)) else actce <-1818/2) else actce <- 0
  #total AC
  ...
  # son age est inferieur à 6 si prescolaire à 1 et plus grand que 6 sinon
  ...
  #Allocation famille
  if (inc<125592/2) (if (inc>49842/2) afce <- max(1000/2, min(2515/2,(2515/2)-(inc-49842/2)*0.04)) else afce <- 2515/2) else afce <- 0
...
où *v* est un terme aléatoire normalement distribué (i.i.d) de moyenne zéro et de variance 1.
...
où $\epsilon_i$ est également un terme aléatoire normalement distribué (i.i.d) de moyenne zéro et de variance $\sigma^2_L$ équivalente à la variance des heures de travail.

```{r}
ErreurU <- list( rep (matrix(rnorm(Lmax/lincr*nsim), Lmax/lincr, nsim), n))
# le terme erreur U sera multiplié par l'écart-type des heures travaillées, tel que sa variance sera égale à l'écart-type au carré.
```
...
Premièrement, nous recodons les fonctions *alpha1*, *Utilite* et *umarginal* pour qu'elles incluent les termes d'erreur. Ensuite, la fonction *wrapi* retourne le nombre d'heures de travail optimales considérant les termes d'erreurs ajoutés.
...
#out <- optim(thetatry, fn=distrhours) # minimise la fonction gmmfct en prenant theta comme valeur de départ.
#thetatry <- out$par
#print(out)
```
