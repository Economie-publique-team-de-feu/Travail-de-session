...
$$\hat{\alpha_{1i}} = \alpha_{1i}+\theta_{12}e^{v_i}$$
où *v* est un terme aléatoire normalement distribué (i.i.d) de moyenne zéro et variance 1. Le paramètre $\theta_{12}$ modifie l'ampleur du terme d'erreur et la transformation $e^{v_i}$ empêche d'avoir une valeur négayive de $\hat{\alpha}_{1i}$
...
```{r}
erreurA <- matrix(rnorm(n*nsim,0,1), nsim, n)
```
...
alphahat<- function(i,theta, sim){
  alphahat <- as.numeric(alpha1(i, theta)) + exp(erreurA[sim,i])*theta[12]
  return(alphahat)
}
...
# calcule l'utilite marginale de l'individu i par increments de incr heures de travail
  ...
  util <- sapply(lsup,function(x) uhat(i,x,theta,sim))
...
```{r}
thetatry <- c(12, -59,  8,  0.5,  10,  5, 1, 4, 0.01, 5, 2, 10)
#gmm de depart
distrhours(thetatry)
