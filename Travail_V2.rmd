...
transferts <- function(inc, i){
  ...
  if (ps[i] == 1) {
    rmg <- rmg
     ...
     rmg <- rmg*1.45 # ajustement pour situtation familiale
  } else if (c[i] == 1) {
     rmg <- rmg*1.87/2 # ajustement pour situtation familiale
  } else if (ce[i] == 1) {
     rmg <- rmg*2.48/2 # ajustement pour situtation familialee
  }
...
* *nsim* est le nombre de simulations que nous allons faire pour ajouter des termes d'erreurs dans le modèle. Nous l'avons fixé à 100. Un nombre trop faible induit des risques que les erreurs induisent trop de *bruit* dans les données et un nombre trop élevé allonge la durée de compilation du code. Nous suggérons de le poser à 10 pour une compilation plus rapide.   
...
thetatry[10]<- thetatry[10]+130
thetatry[11]<-thetatry[11]+20
thetatry[12] <- thetatry[12]-12
...
On voit donc que la majorité des moments sont bien simulés, sauf que le modèle produit un peu trop de variation dans les heures travaillées et que la proportion des gens ne travaillant pas est un peu plus faible que dans les données observées. Voici la distribution des heures travaillées observées et simulées: 
...
Pour les personnes qui ont 20 ans et plus, le revenu minimum garanti correspond a 55% du panier de consommation annuel, évalué a 36970.44 dollars en 2020 pour Montreal (le plus eleve de la province). Le panier de consommation est evalue en dollars constants de 2018. Pour les personnes de 65 ans et plus, le revenu minimum garanti correspond a 100% du panier de consommation de 36970.44 dollars. Les règles fiscales de l'attribution des paniers sont déjà codés dans la fonction `transferts`. Elles consistent à attrubuer universellement un revenu minimal garanti correspondant à un pourcentage du panier de consommation selon le type de ménage et l'âge. Les autres transferts ont tous été retirés, incluant l'aide sociale.
...
hist(stats1[,5], main = "Distribution des revenus nets")
```
...
La prime à l'emploi est de 3$/h pour tout individu ayant trouver un emploi et travaillant au moins 30h par semaine. La prime commence à s'appliquer a 30h de travail durant la semaine. Encore une fois les détails de la réforme sont codés dans la fonction `transferts`. Elle assume que les tranferts habituels sont maintenus, à l'exception de la prime pour les travailleurs (basée sur le salaire annuel) qui est remplacée par la prime au travail (subvention de 3\$ de l'heure). Ce type d'aide au travailleur aura pour effet, théoriquement, d'encourager davantage la participation au travail des individus ayant les plus faibles salaires horaires. 
...
hist(stats2[,5], main = "Distribution des revenus nets")
```
...
hist(stats3[,5], main = "Distribution des revenus nets")
```
...
# essai 1 graphiques en violon
vioplot(stats0[,1],
        stats1[,1],
        stats2[,1],
        stats3[,1], names = c("De base", "RMG", "Prime au travail", "RMG et PT"),
        col=c("lightcyan","cyan1","cyan4", "seagreen"), lty="blank")

# essai 2 graphiques superposes
par(mfrow=c(4,1))
hist(stats0[,1], col="lightcyan", lty="blank")
hist(stats1[,1], col="cyan1", lty="blank")
hist(stats2[,1], col="cyan4", lty="blank")
hist(stats3[,1], col="seagreen", lty="blank")
...
vioplot(stats0[,5],
        stats1[,5],
        stats2[,5],
        stats3[,5], names = c("De base", "RMG", "Prime au travail", "RMG et PT"),
        col=c("lightcyan","cyan1","cyan4", "seagreen"), lty="blank")

# essai 2 graphiques superposes
par(mfrow=c(4,1))
hist(stats0[,5], col="lightcyan", lty="blank")
hist(stats1[,5], col="cyan1", lty="blank")
hist(stats2[,5], col="cyan4", lty="blank")
hist(stats3[,5], col="seagreen", lty="blank")
...
barplot(listeGini,
        names.arg=c("De base", "RMG", "Prime au travail", "RMG et PT"),
        col=c("lightcyan","cyan1","cyan4", "seagreen"), ylim = c(0, 0.20),border=NA)
```
