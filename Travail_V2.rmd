...
2. La mise en place d'une subvention salariale

3. Une combinaison des deux réformes 


Comme mentionné, les effets de ces réformes seront comparés sur la base de l'offre de travail, de la distribution des revenus dans la société et sur les taux marginaux effectifs d'imposition.
...
transferts <- function(inc, i){
  ...
  # Reforme #1: subvention salariale
# Inspirée d'une des options analysées par Clavet et al. (2013) Source : https://www.utpjournals.press/doi/abs/10.3138/CPP.39.4.491
  ...
  if ((inc / wage[i])>30*weeks) {
    subvention <- (inc/wage[i]-(30*weeks))*3 
  } else {
    subvention <- 0 
  }
...
#divise par deux dans le cas des couples (avec et sans enfant) pour avoir le montant obtenu par chaque personne
  ...
  if (reforme == 0) {
    ...
    assign("asc", 1, envir = .GlobalEnv)
    transferts <- TPS_TVH + AllocationTravailleur + ACE + AllocationFamille + CreditSolidarite + subvention
  } else if (reforme == 2) {
    assign("asc", 0, envir = .GlobalEnv)
    transferts <- rmg
  } else if (reforme == 3 ) {
    assign("asc", 0, envir = .GlobalEnv)
    transferts <- rmg + subvention
  }
...
Pour coder l'espérance des 8 moments simulés selon le vecteur de coefficients *theta*, nous utilisons la fonction `simhours`: 
...
On peut donc maintenant lancer la calibration. L'objectif de la calibration est de minimiser la somme du carré des écarts entre  les moments simulés *moments* et les moments observés *rmoments*. L'optimisation de la fonction `distrhours` qui calcule les GMM se fait par la fonction `optim` qui cherche les valeurs de *theta* qui minimisent la fonction. Nous donnons un vecteur de départ arbitraire `thetastart` et l'algorithme d'optimisation retourne le vecteur de `thetatry` qui minimise les GMM. Comme le processus d'optimisation numérique est long, une fois un premier vecteur de paramètres `thetatry` obtenu, nous avons ajusté manuellement les paramètres afin de diminuer la distance entre les moments, lorsque c'était possible. Le code suivant montre également la somme des écarts au carré avant et après le processus d'optimisation et montre une amélioration significative de la prédiction des moments simulés.
...
On voit la valeur calibrée de $\theta$ dans `out$par` et la valeur de la fonction objective dans `out$value`. Par souci de rapidité dans les compilations subséquentes, nous avons enregistré le vecteur `thetatry` séparément. Il n'est pas adéquat d'interpréter les paramètres commes des coefficients économétriques, mais nous pouvons tout de même justifier les paramètres obtenus brièvement. L'age^2 et le fait d'avoir un enfant d'âge préscolaire diminuent l'utilité du loisir. Inversement, le fait d'être une femme et d'être éduqué augmentent l'utilité du loisir. Le loisir au carré diminue l'utilité, témoignant d'une saturation possible de l'utilité. Le revenu net  et le revenu net au carré influencement positivement l'utilité comme attendu. Nous identifions aussi un coût du travail, par le coefficient positif associé au fait de travailler 0 heures, de même qu'une préférence pour le fait de travailler à temps plein. On peut maintenant voir les moments simulés (moyenne sur *nsim* simulations) et les moments réels:
...
La majorité des moments sont bien simulés. Par contre, le modèle produit une plus grande variance des heures travaillées (le 6^e moment). Autrement, la moyenne inconditionnelle des heures travaillées, les moyennes conditionnelles à l'age, au sexe et à l'éducation sont similaires entre les moments simulés et observés. La proportion des gens travaillant à temps plein et ne travaillant pas est également similaire dans les 2 cas. 
...
La masse importante juste avant 2000 heures de travail (i.e. 40 heures par semaine pendant 50 semaines), est intuitive car la plupart des gens travaillent à temps plein. À 0 heures de travail, la distribution est plus élevée qu'une loi normale en raison du coût de travailler (transport, garde des enfants, etc.). Les paramètres permettent d'estimer les moment et par le fait même, de modéliser assez fidèlement la distribution inconditionnelle des heures travaillées. 
...
Dans le scénario de base, une partie des gens ne travaille pas et obtient au minimum 9222$, soit l'équivalent de l'aide sociale et de quelques transferts. Le revenu médian net simulé est de 38048\$ par personne, ce qui est réaliste pour la réalité québécoise. 

Nous illustrons graphiquement les distributions des variables d'intérêt. Comme mentionné, il y a une concentration d'heures travaillées juste sous 2000h par année. Les taxes et transferts normalisent la distribution des revenus bruts. Il est également possible de voir dans le second histogramme l'aire entre les deux courbes qui correspond aux revenus de taxation combinés des gouvernements fédéraux et provinciaux.
...
Nous calculons ensuite le coefficient de Gini, pondéré par les poids populationnels des individus, sur le revenu net afin de décrire les inégalités de richesse dans la société. 
...
Notre coefficient de Gini pondéré de `r Gini0` est beaucoup plus faible (plus égalitaire) que celui qui est calculé par Statistique Canada, qui était de 0.284 en 2016 (http://qceco.ca/n/3739). Ceci s'explique par le fait que les données, même si elles sont pondérées, ne sont pas exactement représentatives de la population, notamment en excluant les étudiants (généralement à plus faibles revenus). Sans avoir à réfléter parfaitement la réalité québécoise, il nous permettra de comparer les réformes au scénario de base (*reforme=0*). 
À partir de ce portrait de la situation de base, il est possible d'introduire des réformes et de visualiser les changements d'offre de travail et de distribuution de revenus qui vont s'opérer. 


# 5. Réforme #1: Subvention salariale
...
La subvention salariale est de 3\$/h pour tout individu ayant trouvé un emploi et travaillant au moins 30h par semaine. La subvention salariale à s’appliquer à 30h de travail durant la semaine. Elle est prise de l’article de Clavet et al. (2013). De cet article, les auteurs font l’hypothèse que la stimulation de l’emploi est la meilleure façon de sortir les agents de la pauvreté matérielle. Ce type d’aide aux travailleurs aura pour effet, théoriquement, d’encourager davantage la participation au travail des individus ayant les plus faibles salaires horaires.
Encore une fois les détails de la réforme sont codés dans la fonction `transferts`. Elle assume que les transferts habituels sont maintenus, à l’exception de la prime pour les travailleurs (basée sur le salaire annuel) qui est remplacée par la subvention salariale de 3$ de l’heure. 
...
Pour un couple avec enfant (modifier le 3e terme *i* pour obtenir les TMEI des autres types de ménages), le graphique suivant prédit les taux marginaux effectifs d'imposition. Ils sont très similaires à ceux présentés dans le scénario de base, incluant le mur de l'aide sociale dans les premières centaines de dollars de revenu. Une particularité est que puisque la subvention salariale est universelle, elle continue de s'appliquer dans les plus hautes tranches de revenu et diminue le TMEI pour tous de façon parallèle au scénario de base à partir de 40000\$ environ. 
...
On peut donc utiliser le modèle pour voir les changements anticipés sur les valeurs des moments.Comme anticipé, la proportion de gens ne travaillant pas a diminuée d'environ 8% à 5% avec la subvention salariale, et ce, même en abolissant l'ancienne allocation pour les travailleurs. La politique atteint l'effet souhaitée d'encourager les gens à travailler à temps plein, y compris pour les femmes. Celles-ci travaillent en moyenne 72 heures de plus par année avec la réforme 1. 

```{r}
print(simhours(thetatry))
```

On peut aussi voir les autres statistiques prédites. Elles sont très similaires à celles du modèle de base en termes d'utilité procurée, de taxes payées, mais les heures de travail moyennes et le revenu médian augmentent.  
...
Voici l'histogramme des heures travaillées et des revenus nets et des TMEI. La subvention salariale contribue à augmenter l'emploi, comme il est possible de le voir dans les graphiques (4000 travailleurs à temps plein vs 3000 sans subvention salariale). La quantité de gens qui travaillent de 2000 à 2500 heures est également plus élevée que dans le programme de base, ce qui veut dire que le nombre de personnes qui travaillent de 40 heures à 50 heures par semaine a augmenté. De ce fait, plusieurs personnes peuvent bénéficier de la prime: elle semble remplir son objectif. Cela est appuyé par le fait que moins de personnes ont choisi de travailler moins de 1000 heures, bien que cet effet est difficile à évaluer parce qu'il semble minime. De plus, il semble y avoir plus de personnes qui ont un revenu net plus élevé considérant que la distribution des revenus nets est plus asymétrique à droite que dans la situation de base.
...
Calculons l'indice de Gini pour la réforme 1. Tous les éléments précédemment mentionnés pourraient expliquer que l'indice de Gini obtenu est infinitésimalement inférieur à celui dans la situation actuelle, bien que nous sachions que le résultat obtenu est déjà plus bas que le réel indice de Gini québécois. 
...
Pour un couple avec enfant, le taux marginal d'imposition avec revenu minimal garanti est beaucoup plus épuré qu'avec la foule de transferts du scénario de base. La principale différence est la disparition du mur de l'aide sociale. Considérant que l'aide sociale ne fait pas partie des transferts dans le scénario avec le revenu minimum garanti, il est logique que ce mur disparaisse. Ceci est dû au fait que le revenu minimum garanti est universel et n’est pas pénalisé en fonction du revenu supplémentaire gagné.
...
On peut utiliser le modèle pour voir les changements anticipés sur les valeurs des moments de la réforme 2.


```{r}
print(simhours(thetatry))
```
L'effet le plus marquant et le plus attendu est que beaucoup plus de gens ne travaillent plus. La proportion de personnes ne travaillant pas est de plus de 20%. L'ensemble de la population travaille moins d'heures en moyenne et les femmes n'y échappent pas, ayant d'avance une plus grance préférence pour le loisir. L'écart-type des heures travaillées s'allonge, témoignant de la plus grande variabilité des choix d'offre de travail suite à la réforme 2.
...
Au dela des heures travaillées et du revenu gagné, on constate que l'utilité minimale des individus a augmenté. Également, même pour l'individu moyen, les transferts (RMG) surpassent les taxes payées. Le revenu minimal atteint 19012\$, soit plus du double de celui du scénario de base. Cette hause de revenu pour les individus les plus défavorisés permet de mieux combler les besoins de base. 


Voici l'histogramme des heures travaillées et des revenus bruts et nets. L'effet notable dans la distribution des heures travaillées est qu'un plus grand nombre de personnes ont choisi de ne pas travailler ou de travailler moins, avec pour résultat qu'un nombre significativement moins élevé de personnes ont choisi de travailler 1500 heures ou plus par rapport à l'option actuelle. C'est un résultat attendu dans le cas de l'instauration d'un revenu minimum puisque les gens gagnent naturellement plus en transferts, suffisamment pour être capables de moins travailler et de pouvoir tout de même subvenir à leurs besoins. Les individus sont ainsi capables de maintenir leur consommation avec moins de travail, ce qui améliore leur utilité pour un niveau de travail donné. Ayant une préférence pour le loisir, ces individus choisissent de moins travailler.  Ensuite, on remarque qu'un nombre minime de personnes gagnent moins de 20 000\$ par année, ce qui est normal considérant que le transfert de base pour quelqu'un qui ne travaille pas et qui a entre 20 et 64 ans est de 20 333\$ par année. De façon générale, la distribution est plus centrée autour de 50 000\$ et le revenu médian a augmenté. Ce sont des résultats plausibles du revenu minimum garanti retrouvés dans la littérature. 
...
Calculons le coefficient de Gini pour la réforme 2. Il est plus faible que dans le scénario de base, mais cela est marginal. Dans l'ensemble le revenu minimum garanti ne diminue que de peu les inégalités.
...
# 7. Réforme #3: Combinaison de RMG et subvention salariale

Dans cette réforme, nous combinons les réformes 1 et 2, soient un revenu minimum garanti et une subvention salariale. Tous les autres transferts, incluant l'aide sociale sont supprimés. 
...
Pour un couple avec enfant, le taux marginal effectif d'imposition est encore une fois simplifié par rapport au scénario de base. Puisque l'aide sociale a été coupée, il y a la disparition du mur de l'aide sociale, où les gens sont taxés à un taux pouvant être supérieur à 1. À la place, on observe un taux marginal effectif d'imposition qui est progressif dans son ensemble, mais décalé à la baisse en raison de la subvention salariale qui rend plus généreux encore les transferts, tous niveaux de revenus confondus.
...
print(simhours(thetatry))
```
L'ajout de la subvention salariale mitige un peu les effets dissuadifs du revenu minimal garanti sur l'offre du travail, avec 19% de personnes ne travaillant pas. 
...
summary(stats3)
```
Avec des transferts encore plus généreux, l'utilité minimale des individus augmente davantage. Le revenu median est encore plus élevé que dans les réformes antérieures grâce à des transferts qui continuent de surpasser les taxes payées même dans le 3e quartile de revenu. 
 

Voici l'histogramme des heures travaillées et des revenus bruts et nets.
...
Concernant la distribution des heures travaillées, une constatation majeure est que le nombre de personnes qui ne travaillent pas a beaucoup augmenté par rapport à la situation actuelle, et cet effet est visible jusqu'à 1500 heures de travail. L'effet du revenu minimum garanti se fait sentir puisque les gens préfèrent avoir plus de loisir tout en sachant qu'ils peuvent réduire leurs heures et tout de même vivre à un niveau convenable en raison du revenu minimum garanti. 

Ensuite, la distribution des revenus nets s'est déplacée pour être centrale autour de 50 000\$, ce qui est beaucoup plus élevé que dans le cas de la situation actuelle. Également, un effet majeur du revenu minimum garanti est qu'il y a un nombre marginal de personnes qui gagnent moins de 20 000\$ par année puisque le transfert minimum du revenu minimum garanti est de 20 333\$ par année. L'effet de la subvention salariale se fait peu sentir tant dans la distribution que sur le revenu des individus, ce qui est espéré puisque le transfert du revenu minimum garanti est plus attirant et potentiellement plus élevé que celui pouvant être fourni par la subvention salariale, d'autant plus que les individus doivent fournir un effort significatif pour obtenir cette prime, donc pour cette raison son effet est plus faible. 



Calculons le coefficient de Gini pour la réforme 3. Encore une fois, celui-ci est marginalement inférieur à celui observé dans la situation actuelle, donc il y a peu de changements à ce niveau.
...
Les réformes sont comparées en utilisant des « graphiques en violons ». La largeur de la forme représente la fréquence (densité) d'observation pour différentes valeurs d'heures travaillées et de revenus nets. Le point central représente la médiane de la distribution et la barre représente l'intervalle interquartile. Ils permettent de comparer rapidement plusieurs distributions de manière élégante. 
...
vioplot(stats0[,1],
        stats1[,1],
        stats2[,1],
        stats3[,1], names = c("Base",  "SubvSal","RMG", "RMG et SubvSal"),
...
En comparant au scénario de base, nous observons que la subvention salariale de 3\$ par heure travaillée à partir de la trentième entraîne une augmentation des heures travaillées. Aussi, il y a un déplacement des individus qui travaillaient moins de 1000 heures vers ceux qui travaillent plus de 2000 heures par année, entraînant une augmentation de la médiane d’heures travaillées. Cependant, lorsque les mesures sont remplacées par le revenu minimum garanti, la quantité de personnes décidant de ne plus travailler augmente de façon importante et diminue le nombre d’heures travaillées dans les portions plus élevées. Elles se concentrent dans la partie inférieure des 2000 heures travaillées par année. L’ajout de la subvention salariale au revenu minimum garanti permet une augmentation des heures travaillées et un aplanissement de la courbe. Dans ce cas, la majorité des individus travaille toujours moins de 2000 heures. Dans l’ensemble, la mesure qui favorise le plus l’offre de travail est, sans surprise, la subvention salariale, soit la réforme 1.
...
vioplot(stats0[,5],
        stats1[,5],
        stats2[,5],
        stats3[,5], names = c("Base",  "SubvSal","RMG", "RMG et SubvSal"),
...
Dans les trois réformes proposées, nous observons une augmentation du revenu net médian. Cependant, la plus grande différence est observée lorsque la réforme du revenu minimum garanti combiné à la subvention salariale est appliquée. Cela est logique et dû au seuil minimum de revenu établi par cette réforme. La subvention salariale à elle seule a tout de même une augmentation de la médiane du revenu intéressante et un amincissement de la queue inférieure de la courbe, ce qui est intuitif et confirme la théorie comme quoi une subvention salariale stimule le travail.

Voici l'effet sur les taux marginaux effectifs d'imposition de chaque réforme proposée:
...
legend("bottomright", legend=c("Base", "Subvention salariale", "Revenu minimum garanti", "RMG et PT"),
       col=c("magenta4", "dark turquoise", "seagreen", "blue2"), lty=c(2,2,1,1), cex=0.8, box.lty=0)
```

Comparativement au scénario de base, nous observons que le mur de pauvreté causé par le programme d’aide sociale disparaît avec le revenu minimum garanti. Cette situation est intuitive puisque le revenu minimum est offert universellement aux agents peu importe leur revenu, il n’est donc pas soumis à des pénalités en fonction du revenu gagné. L’évolution par la suite est moins rapide, mais rattrape le scénario initial à partir d’un peu plus de 100 000\$ de revenus. Un autre élément intéressant est lors de l’application de la subvention salariale. Celle-ci entraîne une diminution parallèle du TMEI comparativement aux scénarios qui ne l’incluent pas. Cette diminution est causée principalement par le fait que, tel que mentionné plus haut, la subvention salariale est un transfert universel qui s’applique à tout niveau de revenu, entraînant ainsi cette diminution parallèle observée des taxes perçues. 


Finalement, voici l'ensemble des indices de Gini, pondérés pour les poids populationnels:

```{r, fig49,fig.width = 4.5, fig.height=3 }
listeGini <- c(Gini0, Gini1, Gini2, Gini3)
barplot(listeGini,
        names.arg=c("Base",  "SubvSal", "RMG","RMG et SubSal"),
        col=c("magenta4", "dark turquoise", "seagreen", "blue2"), ylim = c(0, 0.25),border=NA)
```

Tel que mentionné plus haut, les indices de Gini obtenus dans les simulations sont plus bas que le réel coefficient au Québec, puisque celui-ci a une valeur de 0,26, alors que notre valeur maximale est de 0,209. Cela dit, en comparant les indices de Gini obtenus par les quatre situations comparées, il est possible de constater que la réforme qui offre le coefficient de Gini le plus faible est la subvention salariale, alors que le plus élevé est observé dans la situation actuelle. Cependant, les différences entre les indices sont minimes, variant de 0,01 au maximum. Ce faisant, il serait approprié de conclure que les réformes proposées ne changent pas significativement l'inégalité des revenus. 


## Conclusions
Les résultats de nos simulations démontrent l’arbitrage entre des prestations généreuses et les effets désincitatifs des réformes. Le revenu minimum garanti a un effet considérablement plus grand sur le salaire médian que la subvention salariale. Cependant, le revenu minimum garanti a un fort pouvoir désincitatif sur l’offre de travail, surtout comparativement à la subvention salariale, qui elle agit comme un fort incitatif. La troisième réforme, qui combine ces deux politiques, a un effet encore plus grand sur le revenu médian tout en atténuant légèrement le découragement de l’offre de travail occasionné par le revenu minimum garanti. Par contre, cette combinaison des deux réformes comporte un grand coût pour l’État, la troisième composante de ce trilemme. 

Une force de notre analyse est qu'elle repose sur un modèle d’offre de travail dérivé de la théorie économique. La calibration de notre modèle à partir des données du *Labour Force Survey* pour le Québec par la méthode des moments généralisés montre des résultats très près de ceux observés dans les données. 

D’un autre côté, notre analyse comporte des limites, surtout au niveau des hypothèses simplificatrices effectuées en cours de route. La plupart des transferts ont dû être simplifiés afin d’être codés sans casse-tête ou compte tenu des variables disponibles. Par ailleurs, l’exclusion des étudiants de notre analyse rend l’échantillon légèrement différent de la population québécoise. De plus, nous avons supposé que les données manquantes étaient distribuées aléatoirement afin de les supprimer de notre analyse. Cette hypothèse est difficile à défendre puisque les individus avec des valeurs manquantes diffèrent du reste de la population. Ces deux éléments justifient les écarts observés entre les données et les chiffres officiels, notamment en termes de chômage et d’indice de Gini. Nos réformes ont aussi de très faibles impacts sur les indices de Gini parce qu’une partie des revenus les plus faibles (exclusion des étudiants et absence des données de chômage) et les plus élevés (pour raison de confidentialité des données) sont exclus de l’analyse. 

Pour terminer, une modélisation plus exhaustive des transferts, incluant notamment les pensions canadiennes de la Sécurité de vieillesse pour les individus de 65 ans et plus et davantage de détails sur les âges et sur le nombre d’enfants permettrait d’enrichir cette analyse. Les résultats principaux de notre analyse permettent tout de même de confirmer les tendances répertoriées dans la littérature sur les effets des mesures d’allègement de la pauvreté à la fois sur l’offre de travail et sur la distribution des revenus, et mettent en lumière l’arbitrage nécessaire entre la majoration des transferts de base, l’amélioration des incitatifs au travail et la saine gestion des finances de l’État.  
