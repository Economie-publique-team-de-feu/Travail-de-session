...
alphahat<- function(i,theta, sim){
  alphahat <- as.numeric(alpha1(i, theta)) + erreurA[sim,i]*0
  return(alphahat)
}
...
# calcule l'utilite marginale de l'individu i par increments de incr heures de travail
  ...
  util <- sapply(lsup,function(x) uhat(i,x,theta,sim))+ as.numeric(erreurU[,sim,i])*theta[12]*0
...
machin<- rep(0, n)
for (t in 1:length(machin)) {
  machin[t]<-Lhat(t, theta, 1)
}
hist(machin)
```
